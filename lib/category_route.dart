...
*/
  ...
  Future<void> didChangeDependencies() async {
    ...
    if (_categories.isEmpty) {
      print('_categories is empty...its length is ' +
          _categories.length.toString());
      await _retrieveLocalCategories();
      await _retrieveApiCategory();
    }
    if(_categories[_categories.length-1].units.length==0){
      await _retrieveApiCategory();
    }
  }
  ...
  Future<void> _retrieveLocalCategories() async {
    print('_retrieveLocalCat is called');
  ...
  Future<void> _retrieveApiCategory() async {
    print('_retrieveApiCat is called');
    // Add a placeholder while we fetch the Currency category using the API
    setState(() {
      if(_categories[_categories.length-1].name!='Currency'){
      _categories.add(Category(
    ...
    if (deviceOrientation == Orientation.portrait) {
      return ListView.builder(
        itemBuilder: (BuildContext context, int index) {
          ...
          //print('category '+_categories[index].name +' units is '+
          //  (_categories[index].units.length!=0).toString() );
          return CategoryTile(
            category: _categories[index],
            onTap: _categories[index].units.length != 0 ? _onCategoryTap : null,
          );
        },
      ...
      return GridView.count(
        ...
        children: _categories.map((Category c) {
          return CategoryTile(
            category: c,
            onTap: c.units.length != 0 ? _onCategoryTap : null,
          );
        }).toList(),
      );
    }
 ...
 */
    ...
    return (Scaffold(
      appBar: AppBar(
        ...
        backgroundColor: Color.fromARGB(150, 5, 5, 250),
        elevation: 10.0,
        centerTitle: true,
        toolbarOpacity: 0.1,
      ),
